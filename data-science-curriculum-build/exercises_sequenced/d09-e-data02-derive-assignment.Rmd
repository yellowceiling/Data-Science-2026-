...
df_q4 <-
  ...
  summarize(
    ...
    depth_mean = mean(depth), #depth_mean = mean(depth, na.rm == TRUE)
    depth_sd = sd(depth),
    depth_cov = depth_sd / depth_mean,
    
    diff_mean = mean(diff),
    diff_sd = sd(diff),
    diff_cov = diff_sd / diff_mean,
  )
df_q4
```
...
- Comment on the relative values of `cov_depth` and `cov_diff`.
  - There is not a huge amount of variation in the depth of the diamonds due to the mean being high and the sd being low.
  - There is a lot of variance in the difference between the depth and computed depth.
- Why is `cov_diff` so large?
  - The tiny mean of diff_mean compared to the larger 2.63 sd causes the variance of diff to be extremely high.
