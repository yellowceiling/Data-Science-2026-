...
df_q3 <-
  flights %>%
  filter(
    between(arr_time, (sched_arr_time - 10), (sched_arr_time + 10)) & !(sched_arr_time == arr_time)
  )
```
...
This code doesn't quite do what the user intended---they were trying to filter for only those flights where the desination was Boston airport. What went wrong?
...
**Observations** - Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know? - Out of NYC because all the flights have an origin of an NYC airport whereas there is only 1 flight with a destination of NYC.
