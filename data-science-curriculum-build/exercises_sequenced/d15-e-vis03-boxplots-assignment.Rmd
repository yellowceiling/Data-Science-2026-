...
**Observations** - Which class tends to be most "middle" for `displ` values?

Minivans tend to be in the middle.

\- Which plot is more effective for answering the previous question?

The boxplots make it a lot more apparent. It is very difficult to tell anything with the density plot.
...
diamonds %>%
  ...
  mutate(carat_cut = cut_number(carat, 10)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()

diamonds %>%
  ## TODO: Use mutate to add the column `carat_cut`
  mutate(carat_cut = cut_interval(carat, 50)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()

diamonds %>%
  ## TODO: Use mutate to add the column `carat_cut`
  mutate(carat_cut = cut_width(carat, width = 0.4, boundary = 0)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
