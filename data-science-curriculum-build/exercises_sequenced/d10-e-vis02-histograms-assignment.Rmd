...
- Which graph---`vis1` or `vis2`---best helps you answer each question?
  - First graph helped with the first one
  - Second graph helped with the second one
...
diamonds %>%
  ...
  ggplot(aes(carat)) +
  geom_histogram(binwidth = 0.03, boundary = 0.005) +
  scale_x_continuous(
    breaks = seq(0, 1, by = 0.1)
  )
...
diamonds %>%
  ...
  ggplot(aes(carat)) +
  geom_histogram(binwidth = 0.07, boundary = 0.005) +
  scale_x_continuous(
    breaks = seq(0, 1, by = 0.1)
  )
```
