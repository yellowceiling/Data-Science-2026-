...
**Observations**: - the counts in the hwy values are wrong because instead of creating new bars for overlapping values, the counts keep stacking so without dodge counts get as high as into the 600s whereas using dodge produces counts only as high as almost 50.
...
**Observations**: - The class with the least amount of vehicles are 2seaters with the second lowest class being minivans. The most popular class in the data set is the suv with a count of above 60. The second highest count is the compact cars. Pickup and subcompact are relatively similar in counts.
...
**Observations**: - Majority of suv's are 4 wheel drive, while majority of midsize cars are front wheel drive. Rear wheel drive is the least popular type of drive train. Pickup trucks are entirely 4 wheel drive. Minivans are entirely front wheel drive.

**Compare `v1` and `v2`**:

- Which visualization---`v1` or `v2`---enabled you to make more observations?
  - V2 enabled me to make observations because it supplied me with both the overall counts of the each car class while also showing me the differences in the drive trains.
- What was the difference between `v1` and `v2`?
  - V1 was strictly based on counts of classes while V2 was filled with the drv variable that provided extra data.
...
**Observations** - I hate this graph because the colors are really difficult to tell apart from each other due to being the same value. It is also difficult to read the titles on the x-axis because you're required to tilt your head to one side or be really good at reading sideways text.
